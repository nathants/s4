...
def start(port, conf, extra=''):
    with shell.cd(f'_{port}'):
        for i in range(5):
            try:
                shell.run(f'timeout 60 s4-server -port {port} -conf {conf} {extra}', stream=True)
            except:
                logging.exception('')
                continue
...
def start_all(extra='', num=3):
    ...
    procs = [pool.proc.new(start, port, conf, extra) for port in ports]
    try:
        for _ in range(50):
            try:
                for port in ports:
...
def test_recv_timeout():
    with servers(extra_conf='-max-io-jobs 1', num_servers=1):
