...
s4 ls s4://bucket/dcr/03 || time s4 map \
                             s4://bucket/02_data/ \
                             s4://bucket/dcr/03_selected/ \
                             'bcut 2 | bschema "7*"'

echo; echo 04 bucket and partition
s4 ls s4://bucket/dcr/04 || time s4 map-to-n \
                             s4://bucket/dcr/03_selected/ \
                             s4://bucket/dcr/04_partitioned/ \
                             'bbucket 256 | bpartition 256'

echo; echo 05 sort partitions
s4 ls s4://bucket/dcr/05 || time s4 map \
                             s4://bucket/dcr/04_partitioned/ \
                             s4://bucket/dcr/05_sorted/ \
                             'bsort'

s4 rm -r s4://bucket/dcr/06

echo; echo 06 merge sorted partitions
s4 ls s4://bucket/dcr/06 || time s4 map-from-n \
                             s4://bucket/dcr/05_sorted/ \
                             s4://bucket/dcr/06_merged/ \
                             'bmerge'

echo; echo 07 count and convert to csv
s4 ls s4://bucket/dcr/07 || time s4 map \
                             s4://bucket/dcr/06_merged/ \
                             s4://bucket/dcr/07_counts/ \
