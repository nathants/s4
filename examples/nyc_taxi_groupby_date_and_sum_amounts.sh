...
echo; echo 03 select pickup-date and trim to YYYY-MM
s4 ls s4://bucket/dsa/03 || time s4 map \
                             s4://bucket/02_data/ \
                             s4://bucket/dsa/03_selected/ \
                             'bcut 2,19 | bschema 7*,a:f64'

echo; echo 04 bucket and partition
s4 ls s4://bucket/dsa/04 || time s4 map-to-n \
                             s4://bucket/dsa/03_selected/ \
                             s4://bucket/dsa/04_partitioned/ \
                             'bbucket 256 | bpartition 256'

echo; echo 05 sort partitions
s4 ls s4://bucket/dsa/05 || time s4 map \
                             s4://bucket/dsa/04_partitioned/ \
                             s4://bucket/dsa/05_sorted/ \
                             'bsort'

s4 rm -r s4://bucket/dsa/06

echo; echo 06 merge sorted partitions
s4 ls s4://bucket/dsa/06 || time s4 map-from-n \
                             s4://bucket/dsa/05_sorted/ \
                             s4://bucket/dsa/06_merged/ \
                             'bmerge'

echo; echo 07 count and convert to csv
s4 ls s4://bucket/dsa/07 || time s4 map \
                             s4://bucket/dsa/06_merged/ \
                             s4://bucket/dsa/07_counts/ \
                             'bsumeachf64 | bschema *,f64:a | csv'

s4_cat() { s4 eval s4://bucket/$1 cat; }; export -f s4_cat
echo; echo fetch the results; echo
(
    echo num-passengers num-trips
    s4 ls s4://bucket/dsa/07 -r \
        | awk '{print $NF}' \
